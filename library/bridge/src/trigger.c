                            ...
                            qmi_client_error_type   transp_err)
{
    if(QMI_UIM_REMOTE_GET_SUPPORTED_MSGS_RESP_V01 == msg_id){
        MSG_INFO("QMI_UIM_REMOTE_GET_SUPPORTED_MSGS_RESP_V01\n");
    }
    else if(QMI_UIM_REMOTE_GET_SUPPORTED_FIELDS_RESP_V01 == msg_id){
        MSG_INFO("QMI_UIM_REMOTE_GET_SUPPORTED_FIELDS_RESP_V01\n");
    }
    else if(QMI_UIM_REMOTE_RESET_RESP_V01 == msg_id){
        MSG_INFO("QMI_UIM_REMOTE_RESET_RESP_V01 result:%d, error:%d\n",
        ...
        MSG_INFO("QMI_UIM_REMOTE_EVENT_RESP_V01 result:%d, error:%d\n",
                        ...
                        void *ind_buf, unsigned int ind_buf_len, qmi_txn_handle *txn)
{
    ...
    MSG_INFO("QMI_UIM_REMOTE_CONNECT_IND_V01\n");
    ...
    if(rc != RT_SUCCESS) {
        MSG_INFO("qmi_client_message_decode failed rc = %d\n", rc);
        return rc;
    }
    ...
    MSG_INFO("UIM_REMOTE_CARD_RESET_V01 slot:%d, rc:%d\n", ind_msg.slot, rc);
                        ...
                        void *ind_buf, unsigned int ind_buf_len, qmi_txn_handle *txn)
{
    ...
    if(rc != RT_SUCCESS) {
        MSG_INFO("qmi_client_message_decode failed rc = %d", rc);
        return rc;
    }
                        ...
                        void *ind_buf, unsigned int ind_buf_len, qmi_txn_handle *txn)
{
    ...
    MSG_INFO("QMI_UIM_REMOTE_CARD_POWER_UP_IND_V01\n");
    rc = qmi_client_message_decode(user_handle, QMI_IDL_INDICATION, msg_id, ind_buf,
                        ...
                        void *ind_buf, unsigned int ind_buf_len, qmi_txn_handle *txn)
{
    ...
    MSG_INFO("QMI_UIM_REMOTE_CARD_RESET_IND_V01\n");
    rc = qmi_client_message_decode(user_handle, QMI_IDL_INDICATION, msg_id, ind_buf,
                                    ind_buf_len, &ind_msg, sizeof(uim_remote_card_power_up_ind_msg_v01));
    if(rc != RT_SUCCESS) {
        MSG_INFO("qmi_client_message_decode failed rc = %d\n", rc);
                        ...
                        void                  *ind_cb_data)
{
    ...
    switch(msg_id) {
        ...
        case QMI_UIM_REMOTE_DISCONNECT_IND_V01:
            MSG_INFO("[QMI_UIM_REMOTE_DISCONNECT_IND_V01]\n");
            break;
...
int t9x07_insert_card(uim_remote_slot_type_enum_v01 slot, char *iccid)
{
    ...
    MSG_INFO("t9x07_insert_card slot:%d, rc:%d\n", slot, rc);
    return rc;
}
                                ...
                                uim_remote_card_error_type_enum_v01 error_cause)
{
    ...
    MSG_INFO("t9x07_send_card_error\n");
    ...
    rc = qmi_client_send_msg_async(rm_uim_client, QMI_UIM_REMOTE_EVENT_REQ_V01, &req, sizeof(req),
                                    &resp, sizeof(resp), remote_uim_async_cb, NULL, &txn);
    MSG_INFO("UIM_REMOTE_CARD_ERROR_V01 slot:%d, rc:%d\n", slot, rc);
