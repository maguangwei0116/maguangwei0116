...
int connect_tcp(const char *host_name, const char *addr)
{
    ...
    if (host == NULL) {
        MSG_ERR("Get hostname failed\n");
        return RT_ERR_HTTPS_GET_HOSTNAME_FAIL;
    }

    handle = socket(AF_INET, SOCK_STREAM, 0);
    if (handle == -1) {
        MSG_ERR("Get socket failed\n");
...
int https_init(https_ctx_t *https_ctx, const char *host, const char *port, const char *ca)
{
    ...
    if (res < 0) {
        MSG_ERR("No Certificate found on Client!\n");
    ...
    if (cert) {  // Free immediately
        ...
        MSG_INFO("Subject Name: %s\n", line);
        line = X509_NAME_oneline(X509_get_issuer_name(cert), 0, 0);
        MSG_INFO("Issuer: %s\n", line);
        res = SSL_get_verify_result(https_ctx->ssl);
        if (res != X509_V_OK) {
            MSG_ERR("Certificate verification failed: %d\n", res);
#ifdef TLS_VERIFY_CERT
        ...
        MSG_ERR("No Certificate found on Server!\n");
#ifdef TLS_VERIFY_CERT
