...
int32_t http_parse_url(const int8_t *url,int8_t *host,int8_t *file,int32_t *port)
{
    ...
    if (ptr2) {
        ...
        rt_os_memcpy(host, ptr1, len);
        host[len] = '\0';
        rt_os_strcpy(file, ptr2);
        file[rt_os_strlen(file)] = '\0';
    } else {
        rt_os_memcpy(host, ptr1, rt_os_strlen(ptr1));
        host[rt_os_strlen(ptr1)] = '\0';
    }
    //get host and ip
    ptr1 = strchr(host,':');
    if (ptr1) {
...
static void http_get_ip_addr(char *domain, char *ip_addr)
{
    int32_t i;
...
int32_t http_tcpclient_send(int32_t socket,int8_t *buff,int32_t size)
{
    int32_t sent = 0;
    int32_t tmpres = 0;
...
http_result_e http_post(const int8_t *url,int8_t *post_str,int8_t *header, socket_call_back cb)
{
    ...
    if (rt_os_strlen(post_str) >= BUFFER_SIZE * 4 ||
        url == NULL ||
        header == NULL ||
        cb == NULL){
