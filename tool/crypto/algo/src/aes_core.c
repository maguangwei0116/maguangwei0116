...
static unsigned char g_roundKeyTable[4 * Nb * (14 + 1)];
static unsigned char g_Nr;
...
@brief
  ...
  This function is used for rotation word.

@param[out]  p_word       word
@return
  void
...
*/
/*=========================================================================*/
static void RotationWord(unsigned char *p_word)
{
    unsigned char temp = p_word[0];
    p_word[0]  = p_word[1];
    p_word[1]  = p_word[2];
    p_word[2]  = p_word[3];
    p_word[3]  = temp;
...
static void XorBytes(unsigned char *pData1, unsigned char *pData2, unsigned char nCount)
{
    unsigned char i;
    for (i = 0; i < nCount; i++) {
        pData1[i] ^= pData2[i];
...
unsigned char AES_Init(const void *pKey, unsigned int bits)
{
    ...
    if(bits == AES_BITS_128) {
        g_Nr = 10;
    }
    else if(bits == AES_BITS_192) {
        g_Nr = 12;
    }
    else if(bits == AES_BITS_256) {
        g_Nr = 14;
    } else {
        return 1;
    }
...
{
    if (pbDataIn != pbDataOut) {
        memcpy(pbDataOut, pbDataIn, AES_BLOCK_SIZE);
    }

    if(bMode == AES_ENCRYPT) {
        BlockEncrypt(pbDataOut);
    } else if(bMode == AES_DECRYPT) {
        BlockDecrypt(pbDataOut);
    } else {
        return 1;
    }
    return 0;
