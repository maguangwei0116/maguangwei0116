...
define DOWNLOAD_SVN
	test -e $(DL_DIR)/$($(PKG)_SOURCE) || $(SVN) cat $($(PKG)_SITE) > $(DL_DIR)/$($(PKG)_SOURCE)
endef
...
define DOWNLOAD_NONE
	test -e $($(PKG)_SITE)
endef

define SOURCE_CHECK_NONE
	ls --quiet $($(PKG)_SITE) > /dev/null
endef

define SHOW_EXTERNAL_DEPS_NONE
	echo $(2)
endef
...
define DOWNLOAD_INNER
	$(Q)case "$($(PKG)_SITE_METHOD)" in \
                none) $($(DL_MODE)_NONE) && exit ;; \
        esac ; \
