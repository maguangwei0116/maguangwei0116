    ...
    while(1) {
        if (get_network_state() == NETWORK_CONNECTING) {
            ...
            if (!rt_os_strncmp(g_mqtt_param.opts.rt_channel, "YUNBA", 5)) {
                if ((g_mqtt_param.alias != NULL) && (g_mqtt_param.alias_rc == RT_TRUE)) {
