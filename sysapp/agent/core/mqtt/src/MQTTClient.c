...
thread_return_type WINAPI MQTTClient_run(void* n)
{
    ...
    while (!tostop)
    {
        ...
        if (tostop)
            break;
        //turn on this if you don't care about time and do care about cpu
//        timeout = 1000L;
...
int MQTTClient_set_authkey(char *cid, char *appkey, char* authkey, int *ret_status)
{
    ...
    ret = mqtt_http_post_json((const char *)json_data, "abj-redismsg-4.yunba.io", 8060, "/", (PCALLBACK)get_ret_status);
    if (ret < 0)
        return -1;
...
int MQTTClient_get_authkey(char *cid, char *appkey, char* authkey, int *ret_status)
{
    ...
    ret = mqtt_http_post_json((const char *)json_data, "abj-redismsg-4.yunba.io", 8060, "/", (PCALLBACK)get_authkey_status);
    if (ret < 0)
        return -1;
    *ret_status = retstatus;
