...
void* MQTTPacket_Factory(networkHandles* net, int* error)
{
    ...
    *error = Socket_getch(net->socket, &header.byte);
    ...
    if ((*error = rt_MQTTPacket_decode(net, &remaining_length)) != TCPSOCKET_COMPLETE)
        goto exit; /* packet not read, *error indicates whether SOCKET_ERROR occurred */
    ...
    else
    {
        ...
        else
        {
            ...
            else if (header.bits.type == PUBLISH && header.bits.qos == 2)
            {
                ...
                char *buf = malloc(10);
                buf[0] = header.byte;
                buf0len = 1 + rt_MQTTPacket_encode(&buf[1], remaining_length);
...
int MQTTPacket_send(networkHandles* net, Header header, char* buffer, size_t buflen, int free)
{
    ...
    buf = malloc(10);
    buf[0] = header.byte;
    buf0len = 1 + rt_MQTTPacket_encode(&buf[1], buflen);
#if !defined(NO_PERSISTENCE)
...
int MQTTPacket_sends(networkHandles* net, Header header, int count, char** buffers, size_t* buflens, int* frees)
{
    ...
    buf0len = 1 + rt_MQTTPacket_encode(&buf[1], total);
...
int rt_MQTTPacket_encode(char* buf, int length)
{
    int rc = 0;
 ...
 */
int rt_MQTTPacket_decode(networkHandles* net, int* value)
{
    int rc = SOCKET_ERROR;
    char c;
    int multiplier = 1;
 ...
 * @return the completion code (e.g. TCPSOCKET_COMPLETE)
 */
int MQTTPacket_send_pubrel(uint64_t msgid, int dup, networkHandles* net, const char* clientID)
{
    int rc = 0;
 ...
 * @return the completion code (e.g. TCPSOCKET_COMPLETE)
 */
int MQTTPacket_send_pubcomp(uint64_t msgid, networkHandles* net, const char* clientID)
{
    int rc = 0;
