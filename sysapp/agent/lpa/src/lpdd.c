...
static int parse_install_profile_result(const char* install_result, uint16_t result_len)
{
    // char* bpp_cmd = NULL;
    ...
    */
    ...
    // profileInstallationResultData [39] ProfileInstallationResultData
    result_data_offset = bertlv_get_tl_length(install_result, NULL);
    if (bertlv_get_tag(install_result + result_data_offset) != 0xBF27) {
        MSG_ERR("Could not found profileInstallationResultData! \n");
        return RT_ERR_APDU_STORE_DATA_FAIL;
    }
    element_len = bertlv_get_tlv_length(install_result + result_data_offset);
    MSG_INFO_ARRAY("ProfileInstallationResultData_buf: ", install_result + result_data_offset, element_len);

    // find finalResult
    result_data_offset += bertlv_get_tl_length(install_result + result_data_offset, &element_len);
    final_result_offset = bertlv_find_tag(install_result + result_data_offset, element_len, 0xA2, 1);
    ...
    final_result_offset += bertlv_get_tl_length(install_result + final_result_offset, NULL);

    tag = bertlv_get_tag(install_result + final_result_offset, NULL);
    if (tag == 0xA0) {
        // successResult SuccessResult
        ...
        // errorResult ErrorResult
        final_result_offset += bertlv_get_tl_length(install_result + final_result_offset, &element_len);
        
        // bppCommandId BppCommandId
        bpp_id_off = bertlv_find_tag(install_result + final_result_offset, element_len, 0x80, 1);
        if (final_result_offset == BERTLV_INVALID_OFFSET) {
            MSG_ERR("Profile intatll failed. bppCommandId not found! \n");
            return RT_ERR_APDU_STORE_DATA_FAIL;
        }
        bpp_id_off = bertlv_get_integer(install_result + final_result_offset + bpp_id_off, NULL);

        // errorReason ErrorReason
        error_reson_off = bertlv_find_tag(install_result + final_result_offset, element_len, 0x81, 1);
        if (final_result_offset == BERTLV_INVALID_OFFSET) {
            MSG_ERR("Profile intatll failed. bppCommandId not found! \n");
            return RT_ERR_APDU_STORE_DATA_FAIL;
        }
        error_reson_off = bertlv_get_integer(install_result + final_result_offset + error_reson_off, NULL);

        MSG_ERR("Profile intatll failed. bppCommandId: %d, errorReason: %d. \n", bpp_id_off, error_reson_off);

        // simaResponse
        euicc_response_off = bertlv_find_tag(install_result + final_result_offset, element_len, 0x82, 1);
        if (final_result_offset == BERTLV_INVALID_OFFSET) {
            MSG_ERR("Profile intatll failed. simaResponse not found! \n");
        }
        element_len = bertlv_get_tlv_length(install_result + final_result_offset + euicc_response_off);
        MSG_DUMP_ARRAY("simaResponse: \n", install_result + final_result_offset + euicc_response_off, element_len);
        return RT_ERR_APDU_STORE_DATA_FAIL;
    } else {
