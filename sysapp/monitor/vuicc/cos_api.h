 ...
 *  <0: false
 */
extern result_t cos_client_connect(const void *arg);
 ...
 *  -1: Problem dumping the structure.
 */
extern result_t cos_client_reset(uint8_t *atr, uint8_t *atr_len);
...
/*
 * This function is used to exchange apdu with cos task
 * PARAMETER：
 *   auto_get_resp[IN]: true means auto send get response command to fetch the 61XX data
 *   c_apdu[IN]: command apdu
 *   c_apdu_len[IN]: command apdu length
 *   r_apdu[OUT]: response apdu
 *   r_apdu_len[IN/OUT]: r_apdu buffer size as input and response apdu length as output
 * RETURN VALUES:
 *   0: Success.
 *  -1: Failed.
 */
